...
export function FetchDataCodechef(dir,contestCode){
  ...
  nightmare
    .goto('https://www.codechef.com/COOK44')
...
async function iterateProblems(data) {
    for (let i = 4; i < data.length; i+=2) {
      const problem1 = data[i];
      const x = await testcases(problem1);
      data[i].testCases = x;
      if(i+1<data.length){
        const problem2 = data[i+1];
        const y = await testcases(problem2);
        data[i+1].testCases = y;
      }
    }
    return data;
}
