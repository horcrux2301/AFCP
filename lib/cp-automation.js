...
export default {
  ...
  activate(state) {
    ...
    this.dir = atom.config.get('Codeforces-Automation.directory');
    atom.config.observe('Codeforces-Automation.directory', (newValue) => {
  ...
  compile() {
    ...
    executeCases(tempdir,dirPath,xx) {
      ...
      .then((file) => {
        ...
        // console.log(file[a]);
        // filex = file[a];
        // let inputFileNum = filex.match(/\d/);
        // inputFileNum = filex.indexOf(inputFileNum);
        // let indexOfDot = filex.indexOf('.');
        // let number = filex.substring(inputFileNum,indexOfDot);
        // console.log(number);
        file.map((file) => {
          console.log(file);
          file = file.replace(/ /g,"\\ ");
    ...
    testcases(url,name){
      ...
      rp(url, (a,b,c) => {
        ...
        fs.createReadStream(__dirname + '/template.cpp').pipe(fs.createWriteStream(this.dir + this.contestCode + '/' + name + '/' + name + '.cpp'));
